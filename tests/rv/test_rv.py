...
def test_join_times(times, obs_list, expected):
    assert observationtools.utils.rv_utils.join_times(times, obs_list) == expected
...
def test_strtimes2jd_with_formats(times, format, expected_jd):
    assert np.allclose(observationtools.utils.rv_utils.strtimes2jd(times, reduced=False, format=format), expected_jd)
...
def test_reduced_strtimes2jd(times, expected_jd):
    jd = observationtools.utils.rv_utils.strtimes2jd(times, reduced=True)
